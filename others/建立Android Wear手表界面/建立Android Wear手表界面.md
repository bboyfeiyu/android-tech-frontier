...
Android开发有一个特征，根据用户各个方面的体验来进行开发。在2014年Google I/O大会上，当Android Wear第一次发布的时候，许多开发者和用户认为这不是他们想要的智能手表，同时开发手表界面的官方API还没有发布。鉴于用户需要的是自定义的手表界面，开发者于是在没有文档的提前下破解了Android Wear，找到了创建个性手表界面的方法。
...
幸运地，Google不久后就说，官方API正在开发，并在2014年12月，在开发者社区发布了API。在这篇文章中，我们将学习关于Android Wear的官方API来开发手表界面，实现一个能够按需求扩展的电子手表界面。实现手表界面的方法可能有一点冗杂，但是你能够在[GitHub](https://github.com/tutsplus/AndroidWear-WatchFaces)上找到这篇文章示例应用。
...
第一件你需要做的事，是在Android Studio中建立手表界面项目。
在创建项目时，勾选**Phone and Tablet**，并选择**Minimum SDK**API18，作为操作系统Android 4.3的的最低版本，来支持捆绑的Android Wear应用。你也将需要勾选Wear选项，并在**Minimum SDK**中选择API21.下面有一个例子，**Target Android Devices**窗口是这样的。
...
在如下两个 **Add an Activity** 窗口中，都选择 **Add No Activity**。 
...
在点击了**Finish**后，项目环境就构建完成了，有一个**mobile**模块和另一个**wear**模块。
...
Android Wear通过`WatchFaceService`的使用来实现手表界面。在这篇文章中，你将建立一个`CanvasWatchFaceService`的扩展类，用来实现`WatchFaceService`，也是一个`Canvas`类，用来在手表界面上画画。在Android Studio中，首先在**wear**模块下建立一个新的Java类，继承`CanvasWatchFaceService`。
...
有了这个类后，在该文件下创建一个继承`Engine`的内部类`WatchFaceEngine`。在手表界面中，这个类是用来处理系统事件的引擎，例如窗口的跳转，或者进入ambient mode。
...
完成`WatchFaceEngine`类的代码后，回到外部类，覆写`onCreateEngine`方法，return新建的内部类。这用来关联手表界面服务和引擎，驱动显示。
...
一旦将空的服务和引擎关联后，在一般管理中更新**AndroidManifest**文件，使服务能够被Android Wear发现。记住，当前程序还不能运行。回到上一个类，继续编写引擎。
...
当**wear**清单完成后，需要在**mobile**模块中打开**AndroidManifest.xml**文件，增加两个在**wear**模块中使用的权限`PROVIDE_BACKGROUND`和`WAKE_LOCK`，因为当用户安装手表APK时，Android Wear要求**wear**和**mobile**模块请求相同的权限。当文件都完成后，回到**CustomWatchFaceService.java**类来实现引擎。
...
这个`Engine` 对象是与驱动手表界面的服务相关联的。它处理定时器、显示你的界面、进出ambient mode，和得到手表的显示信息。总而言之，很特殊。
...
第一件事是在引擎中实现一些成员变量，来追踪设备状态、时间间隔和显示的属性。
...
正如所见，定义`TypeFace`，用在数字手表文本中，同时也用在手表界面的背景颜色和文本颜色上。
`Time`对象，用来追踪当前设备时间。因为标准的`WatchFaceService`只追踪在每分钟内的增量，而定义`mUpdateRateMs`用来控制每秒更新手表界面的时间器（`mUpdateRateMs`的值1000ms）。定义`mXffset`和`mYffset`，用来在引擎监测手表物理图形的同时，使得手表界面的显示不太靠近屏幕的顶部或左边，或者被切成一个圆角。定义三个用来追踪设备和应用状况的boolean类型的值，
...
下一个定义的对象是一个broadcast接收器，用来处理用户在旅行的情况下而改变时区。这个接收器清除保存的时区，重置显示的时间。
...
在接收器定义后，在引擎中最后一个创建的对象是一个`Handler`，用来处理每秒更新手表界面。这是很有必要的，因为以上讨论的`WatchFaceService`是有限制的。如果你自己的手表界面只需要每分钟更新一次，你就可以忽视这个部分。
...
Handler的实现很简单。它首先检查信息ID。如果匹配`MSG_UPDATE_TIME_ID`，它销毁当前的视图，然后重画。在视图销毁后，`Handler`检查屏幕是可见的或不在ambient mode。如果是可见的，它发送一个请求，重复下一秒。由此，当手表界面可见并且不在ambient mode时，我们只要在`Handler`中重复动作就可以了，它只要能够在一点点电量时正常刷新每一秒。如果用户没有看屏幕，回到`WatchFaceServcice`的调用，开始每分钟的更新。
...
在示例的app中，如果卡片形式的设置是间断的，你将使用`setWatchFaceStyle`来设置你的通知标签的背景，并显示。你也将设置peek mode，使通知卡片仅占据尽少的空间。
...
最后，当你想按自己的方式显示时，告诉系统不要显示默认的时间。这些只是一小部分可选功能，对于`WatchFaceStyle.Builder`对象，你可以在 [official documentation](http://developer.android.com/reference/android/support/wearable/watchface/WatchFaceStyle.Builder.html)上找到更多的信息。

After your `WatchFaceStyle` has been set, you can initialize mDisplayTime as a new `Time` object.

在`WatchFaceStyle`设置后好，初始化mDisplayTime作为一个新的`Time`对象。
...
`initBackground`和`initDisplayText`会有两个在引擎中定义的`Paint`对象。背景和文本会有颜色设置，文本有文字格式和字体大小设置，同时也有抗锯齿设置。
...
接下来，你需要实现在`Engine`类中的多个方法，它们都是根据设备状态的变化而触发。当用户隐藏或显示手表界面时调用实现`onVisibilityChanged`方法。
...
调用此方法时，它检查手表界面是否是可见或不可见的。如果手表界面是可见的，你定义在`Engine`顶部的BroadcastReceiver会被注册。如果不是，该方法创建一个`ACTION_TIMEZONE_CHANGED`的动作`IntentFilter`，并注册`BroadcastReceiver`监听它。
...
当设备与Android Wear相关联时，`onApplyWindowInsets`会被调用。这通常用来确定手表界面在设备上运行的图形。
...
还需要覆写的方法是`onPropertiesChanged`。当穿戴设备的硬件属性检测完成后，这个方法会被调用，例如，如果设备支持老化的保护或低速ambient mode。
...
在设备初始状态处理后，要实现`onAmbientModeChanged`和`onInterruptionFilterChanged`类。正如名字显示的，当设备是否处于ambient mode，`onAmbientModeChanged`方法会被调用。
...
如果设备处于ambient mode，手表界面的颜色会被改变为黑白色来降低用户电量的流失。当设备从ambient mode中回归，重新设置手表界面的颜色。也要注意在设备要求低的环境中支持抗锯齿。在所有的标志变量设置后，能使手表界面不显示和重绘，然后检查定时器是否应该开启。
...
当用户在可穿戴设备中手动改变中断设置，`onInterruptionFilterChanged `会被调用。调用时，需要检查，如果该设备是响应的，而相应地改变用户界面。在这种情况下，改变手表界面的透明度，如果该设备是无响应的，设置`Handler`只要每分钟更新，然后重画你的手表界面。
...
当设备在ambient mode下，`Handler`定时器会无响应。通过使用定义的`onTimeTick`方法，手表界面仍然可以根据当前的时间每分钟更新。
...
一旦所有的事件处理后，最后就可以画出的手表界面了。`CanvasWatchFaceService `用一个标准的`Canvas`对象，为`Engine`增加`onDraw`，画出手表界面。
...
在本教程中，只是画一个时间的文本表示形式，只要通过改变`OnDraw`来支持模拟的表盘。在这个方法中，要确保更新`Time`对象，然后就可以开始使用手表界面来显示正确时间了。
...
`drawtimetext`是在两个辅助方法帮助下，创建时间文本来显示，并在定义的`onapplywindowinsets`中，将其应用为X和Y的偏移点，。
...
一旦实现了绘画手表界面的方法，就应该去学习所需要的基本知识，来做手表界面。关于Android Wear手表界面最棒的事是，实现任何可能实现的界面。
...
可以在手表或手机上添加活动，取代基于OpenGL实现的`Canvas`，或根据watchfaceservice来写自己的类。
...
去做吧，API或者是信息可以从任何用户的手机中获得，且潜力是永无止境的。在手表界面的创建的过程中获得快乐吧。